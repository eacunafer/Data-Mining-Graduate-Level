...
args(disc.1r)
dbupa=disc.1r(bupa,1:6,binsize=15,out="num")
table(dbupa[,1])
table(dbupa[,2])
table(dbupa[,3])
table(dbupa[,4])
...
parallelplot(iris,name="iris",comb=0)
...
#Deleting mising values
breastw1=na.omit(breastw)
radviz2d(breastw1,name="Breastw")

##################  Plot de coordenadas Estrella #######################
starcoord(breastw1,main="Breastw",class=TRUE)
...
abline(bupapc$rot[1,])
abline(bupapc$rot[2,])
title("Effecto de los componentes principales")
text(4,1.5,"PC1")
text(1,-1,"PC2")
...
print(a)
summary(a)
color=c(1:2)[as.factor(bupa[,7])]
plot(a$x[,1],a$x[,2],col=color)

landsatpc=prcomp(landsat[,-37],scale=T,retx=T)
color=c(1:7)[as.factor(landsat[,37])]
plot(landsatpc$x[,1],landsatpc$x[,2],col=color)
...
#eje1dis=read.table("http://math.uprm.edu~edgar/eje1disc.dat",hea#der=T)
eje1dis
...
abline(indlda/coeflda[2],-coeflda[1]/coeflda[2],lwd=5)
...
#****************************************************************************** 
lda1<-lda(Class~E1+E2,eje1dis,prior=c(.5,.5))
lda1  
 plda1=predict(lda1,eje1dis[,-3])$class
#Matriz de confusion
table(plda1,eje1dis[,3])
  error1=sum(plda1!=eje1dis[,3])
  error1
#**********************************************************************************
#LDA con priors ponderadas
#**********************************************************************************
lda2<-lda(Class~E1+E2,eje1dis)
lda2  
plda2=predict(lda2,eje1dis[,-3])$class
#matriz de confusion
table(plda2,eje1dis[,3])
error2=sum(plda2!=eje1dis[,3])
error2
#*************************************************************************************
...
ldaexa=lda(eje1dis[,1:2],eje1dis[,3],CV=TRUE)
mean(eje1dis[,3]!=ldaexa$class)
ldaveh=lda(vehicle[,1:18],vehicle[,19],CV=TRUE)
mean(vehicle[,19]!=ldaveh$class)
#Por el metodo de validacion cruzada
crossval(eje1dis[,c(1,2,3)],nparts=10,method="lda",repet=10)
...
#Haciendo un scatterplot usando la libreria ggplot2
years=c(3,4,6,7,8,12,15,20,22,26)
ventas=c(9,12,16,19,23,27,34,37,40,45)
library(ggplot2)
#haciendo un dataframe conlos datos de las dos variables datos=as.data.frame(cbind(years,ventas))
#Haciendo el plot
p=ggplot(datos,aes(x=years,y=ventas))+geom_point()
p+ggtitle("Relacion de ventas efectuadas con los años de experiencias del vendedor") + xlab("años") + ylab("ventas")
